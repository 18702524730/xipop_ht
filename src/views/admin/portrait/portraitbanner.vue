...
<div class="clouditembox">
    <h3 class="ptitle">IP画像banner <span class="addbtn" @click="openadd">添加</span></h3>
    <div class='tablebox'>
  <el-table
    ref="multipleTable"
    :data="dataList"
    ...
    <el-table-column
      ...
      align='center'
      min-width="80">
      <template slot-scope="scope">{{ scope.row.ipPortraitCode}}</template>
    </el-table-column>
    <el-table-column
      label="广告图片"
      align='center'
      min-width="100">
      <template slot-scope="scope"><img :src="scope.row.advertisementPic" alt=""></template>
    </el-table-column>
    <el-table-column
      label="广告名称"
      align='center'
    ...
    <el-table-column
      ...
      align='center'
      min-width="80">
      <template slot-scope="scope">{{ scope.row.isShow|onlineFormat}}</template>
    </el-table-column>
    <el-table-column
    ...
    <div class="openbox">
      <el-form label-width="100px" :model="formdata" label-position="right" :rules="rules" ref="formdatas">
          <el-form-item label="广告名称：" prop="advertisementName">
              <el-input v-model="formdata.advertisementName"></el-input>
              <p class="tip">广告名称只是作为辨别多个广告条目之用，不显示在广告中</p>
          </el-form-item>
          <el-form-item label="广告位置：" prop="advertisementPlace">
              IP画像banner
          </el-form-item>
          ...
          <el-form-item label="上线/下线：" prop="isShow">
                <el-radio :label='1' v-model="formdata.isShow">上线</el-radio>
                <el-radio :label='0' v-model="formdata.isShow">下线</el-radio>
          </el-form-item>
          ...
          <el-form-item label="广告链接：">
              ...
              <el-input v-model="formdata.advertisementLink"></el-input>
          </el-form-item>
...
<script>
const addurl = '/api/pictorial/updateIpPortrait'
const delurl = '/api/pictorial/deleteIpPortrait'
const listurl = '/api/pictorial/selectIpPortrait'
  ...
  export default {
    data() {
      return {
        ...
        rules:{
            advertisementName:[{required:true,message:'请输入广告名称',trigger: 'blur'}],
            startTime:[{required:true,message:'请选择开始时间',trigger: 'blur'}],
            endTime:[{required:true,message:'请输选择结束时间',trigger: 'blur'}],
            // advertisementLink:[{required:true,message:'请输入广告链接',trigger: 'blur'}],
        },
        isopen: false,
        multipleSelection: [],
        pgIndex:1,
    ...
    methods: {
      ...
      addone(){
        ...
        data.startTime = this.totimestamp(this.formdata.startTime)
        data.endTime = this.totimestamp(this.formdata.endTime)
        data.advertisementPlace = 'IP画像banner'
        data.advertisementPic = this.adpic
      ...
      handleEdit(index, row) {
        ...
        this.adpic = this.formdata.advertisementPic
        this.title = '编辑广告'
        this.isopen = true;
        // let data ={
        //   id: row.id
