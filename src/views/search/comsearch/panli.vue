                    ...
                    <ul class="rightlist">
                        <li v-for="(item,key) in dataList" :key="key" >
                            <div class="topcont clearfix" >
                                <div class="intr">
                                    <h3><span class="tit" @click="todetail(item)">{{item.inventionTitle}}</span> <span class="collect" v-if="item.isCollect==0" @click.stop="addCollect(item)"><i class="el-icon-star-off"></i>收藏</span><span class="collect" @click.stop="cancelCollect(item)" v-if="item.isCollect==1"><i class="el-icon-star-on shou"></i>取消收藏</span></h3>
                                    <ul class="info">
...
export default {
    ...
    methods:{
       ...
       xuanze(it){
        ...
        todetail(it){
            // let url = CONFIG.indexUrl+'/#/search/panlidetail/3/1/id/'+it.detailId+'?userId='+localStorage.getItem('userId')
            // window.parent.postMessage(
            //     'openTab' + url,
            //     '*'
            // );
            store.set('chaijuexinxi',it)
            this.$router.push({name:'panlidetail',params:{type: 3,islocal: 1}});
        },
       getquery(){
            var re ={};
            if(this.$route.query){
